...
export default function FormChatbot() {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (Array.isArray(data)) {
        ...
        // Update chat history and form data with the server's response
        setChatHistory((prev) => [...prev, { sender: "bot", text: data.reply }]);
        if (data.form) {
          setFormData((prev) => ({ ...prev, ...data.form }));
        }
  ...
  const handleChat = (msg) => {
    if (!msg.trim() || !socket || socket.readyState !== WebSocket.OPEN) return; // Do not send if the message is empty or WebSocket is not ready
    setLoading(true);
    setChatHistory([...chatHistory, { sender: "user", text: msg }]);
